		...
		private JsonSchema GenerateInternal(Type type, Required valueRequired, bool required)
		{
			...
			else if (jsonContract is JsonArrayContract)
			{
				...
				if (collectionItemType != null)
				{
					...
					CurrentSchema.Items.Add(GenerateInternal(collectionItemType, (!flag) ? Required.Always : Required.Default, false));
				}
			}
			else if (jsonContract is JsonPrimitiveContract)
			{
				...
				if (type2.GetValueOrDefault() == JsonSchemaType.Integer && type2.HasValue && type.IsEnum && !type.IsDefined(typeof(FlagsAttribute), true))
				{
