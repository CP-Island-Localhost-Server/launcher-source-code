		...
		public static void DrawMessage(SwrveMessageFormat format, int centerx, int centery)
		{
			...
			for (int i = 0; i < format.Images.Count; i++)
			{
				...
				if (swrveImage.Texture != null)
				{
					...
					Point centeredPosition = swrveImage.GetCenteredPosition(swrveImage.Texture.width, swrveImage.Texture.height, num2, num);
			...
			for (int j = 0; j < format.Buttons.Count; j++)
			{
				...
				if (swrveButton.Texture != null)
				{
					...
					Point centeredPosition = swrveButton.GetCenteredPosition(swrveButton.Texture.width, swrveButton.Texture.height, num2, num);
