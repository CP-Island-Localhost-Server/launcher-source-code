...
namespace Disney.Kelowna.Common
{
	public class ContentManifest
	{
		...
		public struct AssetEntry : IEquatable<AssetEntry>
		{
			...
			public static AssetEntry Parse(string payload)
			{
				...
				while (num2 < payload.Length)
				{
					...
					if (num3 != -1)
					{
						...
						throw new ContentManifestException("Asset entry URI payload contains an unknown query parameter '" + text + "' at index " + num2 + "\n" + payload);
					}
			...
			public static AssetEntry CreateForBundle(string key, string path, string deviceList, string bundleKey, bool isLocalized = false, string defaultLocale = null, bool isCacheOnly = false, string decryptionKey = null)
			{
				Assert.IsNotNull(key);
				Assert.IsNotNull(path);
				Assert.IsNotNull(deviceList);
				Assert.IsNotNull(bundleKey);
				key = key.ToLower();
				path = path.ToLower();
				bundleKey = bundleKey.ToLower();
				deviceList = deviceList.ToLower();
				string extension = Path.GetExtension(key).TrimStart('.');
				string pathPrefix = getPathPrefix(key, path);
			...
			public AssetEntry(string key, string deviceList, string extension, string bundleKey = null, string pathPrefix = null, bool isLocalized = false, string defaultLocale = null, bool isCacheOnly = false, string decryptionKey = null)
			{
				Key = key;
		...
		public struct BundleEntry : IEquatable<BundleEntry>
		{
			...
			public static BundleEntry Parse(string payload)
			{
				...
				return new BundleEntry(key, array, priority);
			}

			public static BundleEntry Create(string key, string[] dependencyBundles, int priority = 0)
			{
				Assert.IsNotNull(key);
			...
			public BundleEntry(string key, string[] dependencyBundles, int priority = 0)
			{
				Assert.IsNotNull(dependencyBundles);
				Key = key;
				Priority = priority;
			...
			{
				bool flag = Key == other.Key && Priority == other.Priority && DependencyBundles.Length == other.DependencyBundles.Length;
				int num = 0;
