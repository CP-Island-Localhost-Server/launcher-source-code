	...
	public class ObjectPool<T> where T : class, new()
	{
		...
		public T Spawn()
		{
			...
			else if (m_fullStrategy != null)
			{
				...
				{
					Log.LogFatal(this, "Pool has exceeded its active instances and there is no replacement object delegate.");
				}
			}
			else
			{
				Log.LogFatal(this, "Pool has exceeded its active instances and there is no full strategy specified.");
			}
			return val;
		}
		...
		protected void AllocateObjects(int amountOfObjectsToAllocate)
		{
			for (int i = 0; i < amountOfObjectsToAllocate; i++)
			{
				...
				{
					Log.LogFatal(this, "Pool could not allocate object for pooling.");
				}
			}
