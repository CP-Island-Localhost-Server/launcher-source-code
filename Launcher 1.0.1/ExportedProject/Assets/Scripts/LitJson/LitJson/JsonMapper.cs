	...
	public class JsonMapper
	{
		...
		private static void AddObjectMetadata(Type type)
		{
			...
			foreach (PropertyInfo propertyInfo in properties)
			{
				...
				{
					ParameterInfo[] indexParameters = propertyInfo.GetIndexParameters();
					if (indexParameters.Length == 1 && (indexParameters[0].ParameterType == typeof(string) || value.IsDictionary))
					{
						value.ElementType = propertyInfo.PropertyType;
					}
				}
				else
				{
					...
					p_data.Info = propertyInfo;
					p_data.Type = propertyInfo.PropertyType;
			...
			foreach (FieldInfo fieldInfo in fields)
			{
				...
				p_data2.Info = fieldInfo;
				p_data2.IsField = true;
				p_data2.Type = fieldInfo.FieldType;
		...
		private static void AddTypeProperties(Type type)
		{
			...
			foreach (FieldInfo info in fields)
			{
				...
				p_data2.Info = info;
				p_data2.IsField = true;
				ProcessAttributes(ref p_data2, info);
				list.Add(p_data2);
			}
		...
		{
			object[] customAttributes = m_info.GetCustomAttributes(true);
		...
		private static object ReadValue(Type inst_type, JsonReader reader)
		{
			...
			else if (reader.Token == JsonToken.ObjectStart)
			{
				...
				if (type == typeof(JsonData))
				{
					((JsonData)obj).SetJsonType(JsonType.Object);
				}
				while (true)
				{
					reader.Read();
					...
					else
					{
						...
						((IDictionary)obj).Add(value, ReadValue(objectMetadata.ElementType, reader));
					}
				}
			}
			return obj;
