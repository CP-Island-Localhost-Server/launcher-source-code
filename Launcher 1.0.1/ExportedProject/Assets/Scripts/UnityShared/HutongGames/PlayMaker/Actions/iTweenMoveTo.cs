			...
			if (transforms.Length >= 2)
			{
				...
				else
				{
					for (int i = 0; i < transforms.Length; i++)
					{
						...
						{
							tempVct3[i] = ((space == Space.World) ? transforms[i].Value.transform.position : ownerDefaultTarget.transform.parent.InverseTransformPoint(transforms[i].Value.transform.position)) + (vectors[i].IsNone ? Vector3.zero : vectors[i].Value);
						}
					}
				}
