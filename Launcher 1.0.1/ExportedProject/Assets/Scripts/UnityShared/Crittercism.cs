...
public class Crittercism : MonoBehaviour
{
	...
	public string[] LogStrippingPatterns = new string[1] { "[0-9]{2}:[0-9]{2}:[0-9]{2}\\s[a-zA-z]{2}" };

	public void Init(IStandaloneErrorLogger errorLogger)
	{
	...
	public static string addErrorPrefix(string logString)
	{
		...
		{
			text += string.Format("{0} {1} ", "[Handled Exception] ", "EXCEPTION");
		}
		else if (logString.Contains("CRITICAL"))
		{
			text += string.Format("{0} {1} ", "[Handled Exception] ", "CRITICAL");
		}
		else if (logString.Contains("FATAL"))
		{
			text += string.Format("{0} {1} ", "[Handled Exception] ", "FATAL");
		}
		else if (logString.Contains("NETWORK_ERROR"))
		{
			text += string.Format("{0} {1} ", "[Handled Exception] ", "NETWORK_ERROR");
		}
		else if (logString.Contains("ERROR"))
		{
			text += string.Format("{0} {1} ", "[Handled Exception] ", "ERROR");
		}
		return text + stripTimestamps(logString);
	}
