	...
	public class XmlNodeConverter : JsonConverter
	{
		...
		private void SerializeGroupedNodes(JsonWriter writer, IXmlNode node, XmlNamespaceManager manager, bool writePropertyName)
		{
			...
			foreach (KeyValuePair<string, List<IXmlNode>> item in dictionary)
			{
				...
				{
					SerializeNode(writer, value2[i], manager, false);
				}
		...
		{
			return (!string.IsNullOrEmpty(elementPrefix)) ? document.CreateElement(elementName, manager.LookupNamespace(elementPrefix)) : document.CreateElement(elementName);
		}
