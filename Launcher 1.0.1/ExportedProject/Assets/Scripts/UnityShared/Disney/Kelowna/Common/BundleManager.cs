...
namespace Disney.Kelowna.Common
{
	public class BundleManager : IDisposable
	{
		...
		public BundleManager(ContentManifest manifest)
		{
			...
			monitorCoroutine = CoroutineRunner.StartPersistent(monitorMountedBundles(), this, "monitorMountedBundles");
		}
		...
		{
			if (mountedBundles.TryGetValue(bundleKey, out mount) && mount != null)
			{
				return true;
			}
			return false;
		...
		private void unmountUnusedBundles()
		{
			foreach (KeyValuePair<string, BundleMount> mountedBundle in mountedBundles)
			{
				BundleMount value = mountedBundle.Value;
