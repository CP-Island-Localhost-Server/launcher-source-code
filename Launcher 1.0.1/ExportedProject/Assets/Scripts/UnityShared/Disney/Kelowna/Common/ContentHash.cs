...
namespace Disney.Kelowna.Common
{
	public class ContentHash
	{
		...
		{
			return CalculateHashForFiles(includeFilenameInHash, new string[1] { filename });
		}
			...
			if (jsonData != null)
			{
				...
				else if (jsonData.IsObject)
				{
					...
					for (int i = 0; i < list.Count; i++)
					{
						...
						JsonData value = deterministicallySortJsonData(jsonData[prop_name]);
						jsonData2[prop_name] = value;
					}
					jsonData = jsonData2;
				}
			}
