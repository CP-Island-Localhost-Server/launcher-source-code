...
{
	public static class ClientInstalledCheck
	{
		...
		public static string GetInstalledVersion()
		{
			...
			if (clientCurrentlyInstalledLocation == null)
			{
				return null;
			}
			string path = Path.Combine(clientCurrentlyInstalledLocation, "ClubPenguinIsland.exe");
			if (File.Exists(path))
			{
				result = getRegistryValue("DisplayVersion");
			}
			return result;
		}
		...
		private static string getMacInstalledVersion(string currentlyInstalledLocation)
		{
			...
			{
				Log.LogErrorFormatted(typeof(ClientInstalledCheck), "Error installing client: {0}", ex.Message);
			}
			...
			if (process.StartInfo.RedirectStandardError)
			{
				...
				{
					Log.LogError(typeof(ClientInstalledCheck), text2);
				}
			}
			return result;
		}
