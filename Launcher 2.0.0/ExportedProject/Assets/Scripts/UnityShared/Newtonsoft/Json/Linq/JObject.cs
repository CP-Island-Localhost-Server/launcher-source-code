...
{
	public class JObject : JContainer, IDictionary<string, JToken>, ICollection<KeyValuePair<string, JToken>>, IEnumerable<KeyValuePair<string, JToken>>, IEnumerable, INotifyPropertyChanged, ICustomTypeDescriptor, Newtonsoft.Json.ObservableSupport.INotifyPropertyChanging
	{
		...
		private static Type GetTokenPropertyType(JToken token)
		{
			if (token is JValue)
			{
				...
				return (jValue.Value != null) ? jValue.Value.GetType() : typeof(object);
			}
