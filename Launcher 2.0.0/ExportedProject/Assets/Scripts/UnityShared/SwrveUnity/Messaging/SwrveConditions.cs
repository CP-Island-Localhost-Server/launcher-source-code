...
namespace SwrveUnity.Messaging
{
	public class SwrveConditions
	{
		...
		private bool matchesEquals(IDictionary<string, string> payload)
		{
			...
			return triggerOperatorType.GetValueOrDefault() == TriggerOperatorType.EQUALS && triggerOperatorType.HasValue && payload.ContainsKey(key) && string.Equals(payload[key], value, StringComparison.OrdinalIgnoreCase);
		}
		...
		public static SwrveConditions LoadFromJson(IDictionary<string, object> json, bool isRoot)
		{
			...
			string text = (string)json["op"];
			if (text == "eq")
			{
				string text2 = (string)json["key"];
				string text3 = (string)json["value"];
				if (string.IsNullOrEmpty(text2) || string.IsNullOrEmpty(text3))
				{
					return null;
				}
				return new SwrveConditions(TriggerOperatorType.EQUALS, text2, text3);
			}
			if (isRoot && text == "and")
			{
